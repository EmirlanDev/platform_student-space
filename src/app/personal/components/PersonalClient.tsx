...
import { useAdminSelect, useBurger } from "@/zustand/allState";
import AdminNavLink from "@/NavLinks/AdminNavLink";
...
export default function Personal({ children }: PersonalProps) {
  ...
  //! Zustand
  const { openBurger, closeBurger, isOpenBurger } = useBurger();
  const { openCloseAdminSelect, isOpenAdminSelect } = useAdminSelect();
  ...
  return (
    <div className="min-[1024px]:flex">
      <button
        onClick={openBurger}
        className="fixed z-4 min-[1024px]:hidden flex items-start w-[58px] max-[600px]:w-[52px] h-[50px] max-[600px]:h-[45px] p-[13px] flex flex-col justify-between hover:bg-[#1d53c520] rounded-lg mt-[20px] ml-[27px] max-[700px]:ml-[14px]"
      >
      ...
      ></div>
      <aside
        className={`panel fixed overflow-y-scroll bg-[#1d53c5] max-w-[370px] transition-[.5s] inset-[0] w-[40%] min-w-[300px] bg-[#1D53C5] py-[50px] max-[900px]:py-[20px] px-[40px] max-[900px]:px-[20px] z-[200]  max-[800px]:rounded-r-[10px] ${
          isOpenBurger
      ...
      >
        ...
        <nav>
          <ul className="flex flex-col text-[20px] max-[800px]:text-[18px] font-[500] py-[57px] max-[800px]:py-[28px] text-white">
            {data?.isAdmin ? (
              <li
                onClick={openCloseAdminSelect}
                className="flex justify-between items-center hover:bg-[#ffffff40] max-w-[290px] py-[10px] px-[40px] max-[800px]:px-[20px] max-[800px]:py-[5px] rounded-lg cursor-pointer"
              >
                Админ
                <span className="w-[10px] h-[10px] border-t-[2px] border-r-[2px] rotate-135 mb-[4px]"></span>
              </li>
            ) : null}
            {isOpenAdminSelect
              ? [
                  {
                    path: data?.isAdmin ? "/personal/admin/news" : "",
                    title: data?.isAdmin ? "Новости +" : null,
                  },
                  {
                    path: data?.isAdmin ? "/personal/admin/event" : "",
                    title: data?.isAdmin ? "Мероприятия +" : null,
                  },
                  {
                    path: data?.isAdmin ? "/personal/admin/networking" : "",
                    title: data?.isAdmin ? "Нетворкинг +" : null,
                  },
                ].map((el, idx) => (
                  <AdminNavLink onClick={closeBurger} href={el.path} key={idx}>
                    {el.title}
                  </AdminNavLink>
                ))
              : null}
            {[
              ...
              <NavLink onClick={closeBurger} href={el.path} key={idx}>
                {el.title}
              </NavLink>
            ))}
          </ul>
